...
Track C shifts the focus from heuristic construction and benchmarking (Track B) to a **fully integrated extractive summarization pipeline**, combining silver-label generation and LLM-based sentence extraction within a reproducible evaluation framework.

#### Objectives

The primary goal of this track is to:

* Operationalize the best-performing heuristic identified in Track B;
* Generate large-scale **silver extractive annotations** from purely abstractive datasets;
* Benchmark multiple prompting strategies under controlled and reproducible conditions;
* Evaluate extractive predictions against both silver labels and abstractive references.

This track therefore bridges heuristic supervision and LLM-based extraction into a unified experimental pipeline.
...
1. **Heuristic Selection**
   The system reads the results stored in `best_heuristic.json` and automatically selects the optimal heuristic configuration.
   If available, the parameter `best_k_single` is used as a fixed upper bound for sentence selection, allowing grid search to be skipped.

2. **Silver Label Construction**
   Using the selected heuristic, abstractive summaries from:

   * **CNN/DailyMail**
   * **XSum**

   are converted into their corresponding extractive “silver” sentence indices.
   The resulting dataset is saved in **Parquet format** for efficient downstream processing.

3. **LLM-Based Sentence Selection**
   Multiple prompting strategies are employed to predict extractive sentence indices from the source documents, including:

   * Vanilla prompting
   * Least-to-Most prompting
   * Tool-augmented prompting
   * Scoring-based prompting
   * Self-Ask prompting

   Each strategy enforces a strict constraint:

   > The model must select **at most K sentences**, where K is determined by the selected heuristic.

---

#### Few-Shot Prompting (Optional)

Few-shot prompting is implemented as an isolated and modular component (`fewshot.py`).

* Exemplars can be generated from a validation silver set (`make-shots`);
* Any prompting strategy can be wrapped at benchmarking time using:

  * `--shots-path`
  * `--shots-n`
  * `--shots-seed`

This design ensures that few-shot conditioning remains reproducible and decoupled from the core extraction logic.

---

#### Evaluation and Benchmarking

Performance is evaluated using two complementary perspectives:

1. **Extractive Accuracy (against silver indices)**

   * Precision
   * Recall
   * F1-score

2. **Content Overlap (against abstractive reference summaries)**

   * ROUGE-1
   * ROUGE-2
   * ROUGE-L (F1)

All evaluation results are stored as structured JSON reports containing full experimental provenance, including:
...
This ensures transparency and replicability of every experiment.

---

#### Reproducibility and Robustness

To guarantee experimental stability:

* Random seeds are fixed across all components;
* Sentence splitting includes a robust fallback mechanism;
* Model loading includes a public fallback configuration (e.g., Qwen variants) in case the requested model cannot be accessed.

Supported instruction-tuned models include:
...
Track C thus represents the culmination of the project: a scalable, reproducible framework for extractive summarization that integrates heuristic supervision, silver-label generation, and advanced prompting strategies within a unified benchmarking environment.
